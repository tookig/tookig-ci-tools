...
(function (controls) {
  ...
  function dragDropList (args) {
    ...
    const iargs = Object.assign({
      dragDrop: function (item, from, to) {}
    }, args)
    ...
    const list = controls.itemList(iargs)
  ...
  function footerOverride (footer) {
    return function () {
      const tbody = footer.call(this)
      tbody.find('tr').each(function (i, element) {
        const tr = $(element).find('td').first()
  ...
  function insertOverride (insert) {
    return function (item) {
      // Get row
      const tr = insert.call(this, item)
      // Add grip handle
      $('<td></td>').append(
        $('<span></span>').addClass('ui-icon ui-icon-grip-dotted-horizontal')
      ).prependTo(tr)
      return tr
    }
  }
